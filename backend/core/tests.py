...
class AdminSiteSmokeTests(TestCase):
    ...
    def test_every_registered_model_add_form_loads(self):
        ...
        for model in admin.site._registry:
            ...
            response = self.client.get(url)
            expected = 403 if model._meta.label in {"applications.Application", "applications.ApplicationMessage", "applications.RetainedApplicationReview", "applications.RetainedApplicationReviewCandidate", "documents.Category", "documents.CategoryMembership", "documents.FolderOverride", "documents.Document", "documents.DocumentOverride", "documents.DocumentExtraction", "email_sync.MailboxLineage", "email_sync.MailboxPrincipal", "email_sync.AccountMailboxBinding", "email_sync.RetainedMessage", "email_sync.RetentionKey", "email_sync.RetainedObservation"} else 200
