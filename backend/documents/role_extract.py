...
Django port note (Phase 8 slice, docs/DJANGO_MIGRATION_PLAN.md): ported
byte-for-byte from _app/role_extract.py. This module is pure text/regex
logic with zero filesystem or framework dependency, so unlike
extract.py (which had to move from path-based reads to Document.file
storage reads) there is nothing here for the port to change.
