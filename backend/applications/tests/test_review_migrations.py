"""Additive review upgrade from b5c1faf on disposable SQLite; no real tracker writes."""
import tempfile
...
class ReviewMigrationTests(TransactionTestCase):
    def test_populated_checkpoint_preserved_without_review_backfill(self):
        with tempfile.TemporaryDirectory() as temp:
            alias = "application_review_fixture"
            ...
            try:
                ...
                self.assertNotIn("applications_retainedapplicationreview", db.introspection.table_names())
                ...
                for n in range(2):
                    ...
                    for attempt in range(2):
                        ...
                        create("applications", "ApplicationMessage", workspace_id=ws.pk, application_id=application.pk, retained_message_id=message.pk)
                ...
                executor = MigrationExecutor(db)
                migration = executor.loader.get_migration("applications", "0008_retained_application_review")
                ...
                self.assertEqual(executor.migration_plan(leaves)[0][0].name, "0008_retained_application_review")
                executor.migrate(leaves)
                new = executor.loader.project_state(leaves).apps
                for model in ("RetainedApplicationReview", "RetainedApplicationReviewCandidate"):
                    self.assertEqual(new.get_model("applications", model).objects.using(alias).count(), 0)
                for table in tables:
