"""
Deterministic job-posting extraction, ported from _app/posting_extract.py
as part of Phase 6 (docs/DJANGO_MIGRATION_PLAN.md) -- landing the
LinkedIn/Handshake/Indeed/Lensa fixture-validated parsers onto the new
JobPosting model. The business logic below is an unmodified, like-for-
like port: same providers, same body-parsing rules, same dedupe-key
scheme. What changed is only where it lives and what calls it --
postings/services.py's ingestion glue turns the raw dicts this module
returns into real JobPosting rows.

In one line: a job-alert/digest email (LinkedIn, Handshake, ...) can
...
Layered design:
  ...
  Layer 4 -- URL evidence:         attached by the caller, not this module

Four providers have real multi-job block parsers, validated against real
fixture emails in postings/tests/fixtures/email-source/: LinkedIn,
Handshake, Lensa, and Indeed (Indeed dispatches to one of two body
parsers by subject shape -- see _parse_indeed()'s docstring). A second,
separate tier -- Layer 3b below -- covers senders where one email is
always exactly one job (Honeywell, McDonald's/jobs2web, AWS Educate,
...
extract_postings() correctly returns [] for it. Adding a new multi-job
provider means adding one `_parse_<provider>()` function and one branch
in extract_postings(); adding a new single-job provider means adding a
sender hint and a provider id to _SINGLE_JOB_FALLBACK_PROVIDERS.
"""
