...
def build_flow(state: str | None = None, code_verifier: str | None = None) -> Flow:
    ...
    `code_verifier`, when given, is applied to the flow *before* any
    call that would otherwise auto-generate one. This is what lets
    complete_gmail_connection() reuse the exact PKCE verifier that
    build_authorization_url()'s flow generated: Flow.authorization_url()
    auto-generates a code_verifier and derives the code_challenge sent
    to Google from it, but that verifier lives only on the in-memory
    Flow object -- it's never encoded into the authorization_url or
    the state Google echoes back. Without explicitly threading it
    through here, the callback-time flow would generate its own
    unrelated verifier and Google's token exchange would correctly
    reject it as not matching the challenge it was originally given."""
...
def build_authorization_url() -> tuple[str, str, str]:
    """Returns (authorization_url, state, code_verifier). Both `state`
    and `code_verifier` must be stashed by the caller (GmailConnectView
    stores them in the session, alongside which workspace initiated
    the connect) and fed back in at callback time.

    `state` is checked against what comes back on the callback
    request, standard OAuth CSRF protection -- without it, an attacker
    who can get a victim to visit a crafted callback URL could bind
    their own Google account to the victim's session.

    `code_verifier` is PKCE's proof-of-possession secret: the
    authorization_url this function returns embeds a code_challenge
    derived from it, and Google's token endpoint will refuse to
    exchange the resulting authorization code for tokens unless the
    *same* verifier is presented at that point -- see build_flow()'s
    docstring for why that means this value has to survive the round
    trip the same way state does, rather than being regenerated by a
    fresh Flow at callback time."""
    flow = build_flow()
...
def complete_gmail_connection(
    workspace, code: str, state: str, code_verifier: str | None = None
) -> EmailAccount:
    ...
    of the session, same as state and workspace_id) -- see
    build_flow()'s docstring for why a freshly-built flow can't just
    regenerate its own.
    """
