...
index so you don't have to open all five to find the right one.

| Script | Use when |
|---|---|
| `backfill_job_posting_urls.py` | Existing tracker has `job_postings` rows with `posting_url = NULL` from before the `utm_`-filter / HTML-link fixes (see `AUDIT_FINDINGS.md`, Findings 4 & 6). Rewrites those rows in place. |
