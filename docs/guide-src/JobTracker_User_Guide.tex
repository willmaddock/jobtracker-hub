...
The switcher offers up to three ways to add a tracker, depending on how you're running JobTracker Hub:

\begin{itemize}
  \item \app{Create New Tracker} starts a fresh, empty tracker as a sibling folder next to your original one.
  \item \app{Import a Copy} brings an existing folder of documents or a \file{.zip} export into a new, independent tracker. Use the dropzone for a \file{.zip}, or choose \app{Choose a folder instead} for a folder. In the packaged desktop app, the folder path uses the same preview-before-commit safety step described in Section~3.2.
  \item \app{Use an Existing Folder} (desktop app only) points a new tracker straight at a folder in place — nothing is copied. It's the same pick-preview-confirm flow as first launch, just reachable again later from the switcher.
\end{itemize}
...
\screenshot[0.5\textwidth]{images/new-import-panel.png}{The switcher's \app{Import a Copy} panel, with both \file{.zip} and folder import options.}
