...
\textbf{Guide version:} 1.7 \quad | \quad \textbf{Updated:} August 28, 2026\par
\vspace{0.6cm}
...
\subsection{Installing the macOS desktop app}
...
Unlike the command-line install, the desktop app writes its data to a per-user Application Support folder rather than next to the code, since the installed \file{.app} bundle itself is read-only:
...
\screenshot[0.9\textwidth]{images/search-hub.png}{Part of the AI-powered job search \& career assistants row in Search Hub.}
...
\begin{itemize}
  ...
  \item \app{Import a Copy} brings in an existing folder of documents as a new, independent tracker. Drag a \file{.zip} export onto the dropzone (or click to browse to one), or click \app{Choose a folder instead} to pick a folder directly — picking a folder shows the same preview described in Section~3.2 before anything is copied, so there's no need to also use the \app{Import a Copy} button for folders; that one is for \file{.zip} files only.
...
\textit{Revision 1.2} removes the Streamlit interface from this guide. The repository's \file{app.py}, \file{ui.py}, and \file{views_*.py} were deleted; the two label dicts the web app actually depended on moved to a new, dependency-free \file{labels.py}. The web app was already the more capable interface (see Section~4) and never needed Streamlit to run — \file{api.py} serves \file{frontend/index.html} itself, so \textit{running it} is now a single command and a single address rather than a choice between two interfaces.

\textit{Revision 1.3} adds Section~3.2, \textbf{Installing the macOS desktop app}, covering the \file{.dmg} drag-to-Applications install flow and first-launch folder picker, and adds a Troubleshooting entry for the desktop app's startup log and the new Settings~\textgreater~Diagnostics panel.

\textit{Revision 1.4} documents the \app{Learn More} and \app{User Guide} shortcuts added to the installer window below the main drag-and-drop row, and redraws the installer-window figure with higher-contrast labels.

\textit{Revision 1.5} corrects the installer-window figure and \file{package-dmg.sh} icon positions: the \app{Optional extras} row was drawn 40px lower than where Finder actually places those icons, causing the row-2 icons to overlap the caption above them on a real build. Verified against a screenshot of an actual install.

\textit{Revision 1.6} rewrites the Gatekeeper warning box in Section~3.2 to match the actual first-launch dialog (titled "Not Opened," offering only \textbf{Move to Trash} and \textbf{Done} — no \textbf{Open Anyway} button), replacing the previous "App is damaged" wording. Adds the explicit \textbf{Done}, then right-click \textbf{Open}, then \textbf{System Settings \textgreater{} Privacy \& Security \textgreater{} Open Anyway} fallback sequence, verified against screenshots of an actual blocked launch.

\textit{Revision 1.7} updates Section~3.2's first-launch description and adds a screenshot of the folder preview step, which was previously undocumented. Adds a new \textbf{Search Hub} subsection to Section~4 describing the external job-board directory, target role/AI prompt, and custom cards, with a screenshot of the AI-assistant cards. Rewrites Section~7.1 (\textbf{Creating and importing trackers}) to match the switcher's current button labels (\app{Create New Tracker}, \app{Import a Copy}, and the desktop-only \app{Use an Existing Folder}) and documents the folder-preview step shared with first launch. Replaces the tracker-switcher and import-panel figures, both of which still showed a retired "New tracker"/"Import tracker" button design. Adds a Settings~\textgreater~Diagnostics screenshot to Section~10.1.
