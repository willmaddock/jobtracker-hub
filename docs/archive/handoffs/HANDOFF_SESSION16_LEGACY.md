...
Last updated: 2026-09-01 (session 16), by Claude.

**Session 16 — built Part 3 of discoveries-kanban-spec.md (the frontend
board); Parts 1/2/4 were already done and are now confirmed against the
real code, not just assumed.** This session started a fresh conversation
with no memory of any prior chat's "recommendations" — the person
referenced approving recommendations that were never actually given in
this thread, so before writing anything this session read the uploaded
`jobtracker-hub-part1-backend.zip` end to end (not just skimmed) to find
out what was actually true on disk:

- **Part 1 (backend `kind` classification) — confirmed already shipped.**
  `overrides_store.py`'s `discovered_matches` table has the `kind` column
  (`'application'`/`'posting'`) with a migration
  (`test_migration_adds_kind_column_to_a_pre_kanban_db`), and
  `mail_app_store.is_job_posting_style_subject()` catches all three
  patterns from the spec (`just posted a \d+% match`, `new .* jobs open`,
  `just in:.*has new`) with tests in `test_mail_app_store.py`.
  `api.py`'s `sync_account()` and `discover_new_applications()` both set
  `kind="posting"` (and clear `candidate_item_keys`) for postings so they
  never enter the ambiguous-application queue —
  `test_sync_routes_job_posting_shaped_company_only_hit_to_postings_not_ambiguous_queue`
  covers the exact KPMG-shaped case from the spec.
- **Part 2 (attach unchanged, mark-posting added) — confirmed already
  shipped.** `POST /api/discoveries/{id}/attach` untouched.
  `POST /api/discoveries/{id}/mark-posting` exists
  (`ov.set_discovery_kind`), tested in `test_discoveries.py`
  (relabels a pending discovery, 404 on unknown id, 409 if already
  decided).
- **Part 4 (never let accept create a duplicate folder) — confirmed
  already shipped, and confirmed working from the person's own real
  walkthrough.** `build_index.create_application_folder()` raises
  `FileExistsError` when the destination already exists and is
  non-empty; `accept_discovery()` in `api.py` catches that and returns
  409 (not a warn-and-proceed). The person's own terminal log from the
  live walkthrough shows this actually firing: `POST
  /api/discoveries/49/accept HTTP/1.1" 409 Conflict` against the
  already-existing `Applications/Kp` folder — this is real, observed
  backend behavior, not just code review.
- **Part 3 (frontend Kanban board) — this is what was actually
  missing**, and is what this session built. Confirmed its absence
  first (`grep`'d the whole `index.html` for `DiscoveriesBoard`/
  `KanbanBoard` and found only the Pipeline page's `KanbanBoard`) —
  Email Sync was still the flat list/dropdown UI from the person's own
  screenshots (which matches Part 4's write-up: that walkthrough was
  explicitly run against the pre-Part-3 UI to confirm the design before
  building it).

**What Session 16 built, concretely (all in `_app/frontend/index.html`,
no backend changes needed — every action the new board takes was
already exposed by an existing endpoint):**

- `DiscoveriesBoard` — the 3-column board (Job Postings / Needs Triage /
  Applications), a straight clone of `KanbanBoard`'s native-HTML5 drag
  mechanics (`dataTransfer`, `dragId`/`overCol` state, `onDragOver`/
  `onDrop` per column) rather than hand-rolled drag events, per the
  spec's explicit instruction. Job Postings sorts newest `received_at`
  first by default with a click-toggle for oldest-first. Needs Triage
  merges the old two row shapes (plain "Review" rows and "Which
  application?" rows) into one queue, same as the spec calls for.
  Applications is a drop target only, not a persisted card list — since
  attaching/accepting immediately resolves a discovery (removes it from
  the pending list the API returns), there's nothing to render there as
  a static card; dropping a Needs-Triage card on it opens the resolver.
- `DiscoveryCard` — renders quick-pick chips directly on the card when
  `candidate_item_keys` is non-empty (click a chip = immediate
  `/attach`, no modal — the common KPMG/Comcast case never needs to
  leave the card), an "… more" link to the full resolver for anything
  not covered by those 2–3 chips, a "Pick application…" button when
  there are no candidates at all, "Mark as posting" (click-equivalent
  of dragging onto Job Postings), "View email"/"Dismiss"/"Dismiss all
  N from this sender" carried over unchanged from the old row.
- `ResolveDiscoveryModal` — opened by clicking "Pick application…" /
  "… more", or by dropping a card onto the Applications column. Shows
  the backend's narrowed candidates (if any) as the same chip buttons,
  plus a live-filtered search box over *every* open application (not
  just the backend's candidates), plus "+ Create new application for
  '<guessed company>'" which hands off to the existing
  `DiscoveryReviewModal` (unchanged) for the true-new-company case —
  this is the "type-ahead combobox ... include a Create new application
  option" the spec asked for; there was no existing type-ahead
  component in the codebase to reuse, so this is new, built to match
  the spec's described behavior rather than any prior UI.
- Every drag action has a click equivalent, per the spec's explicit
  requirement: chip click = drag-to-a-chip, "Pick application…"/"…
  more" click = drag-to-Applications, "Mark as posting" click =
  drag-to-Job-Postings.
- `EmailHubPage` gained a List/Board toggle (mirroring the existing
  Pipeline page's List/Kanban toggle pattern), defaulting to Board.
  This isn't in the spec, which describes the board as a straight
  replacement — added anyway as a low-cost safety net: if the new board
  has an undiscovered bug, the person can click back to the exact same
  list UI that was already verified end-to-end in the person's own
  walkthrough, without losing any functionality. The bulk
  select/dismiss/attach toolbar (checkboxes, "Select all", "Dismiss
  selected", "Attach selected") is List-view-only, unchanged from
  before — the board's per-card actions are the batch story there
  instead, per the spec's "click as a first-class trigger... so
  triaging a large batch means clicking through them" framing.
- `EmailHubPage` now receives `apps` as a prop from `App()` (needed for
  the resolver's search-all-applications list) — the only change
  outside `EmailHubPage`/the new components themselves.

**Verification this session:** same constraint as prior sessions — this
sandbox has no network access, so `pip install` and a real `pytest` run
were not possible; Parts 1/2/4 are "confirmed by reading the actual
shipped code + the person's own terminal log," not re-run by me this
session. This sandbox *did* have `esbuild` available locally (via an
already-installed `tsx` package's vendored binary, no network needed),
so the entire `<script type="text/babel">` block was extracted and run
through `esbuild`'s JSX parser both before touching the file (clean
baseline) and after every edit (clean) — confirms syntax only, not
runtime/logic correctness. Manually walked every hook call
(`useState`/`useEffect`/`useMemo`/`useRef`) in `EmailHubPage`,
`DiscoveriesBoard`, `DiscoveryCard`, and `ResolveDiscoveryModal` in
source order, confirming none sit after a conditional return — this is
the exact Rules-of-Hooks bug session 15 hit and fixed elsewhere in this
same file, worth checking explicitly given it's not caught by a syntax
parse. **Not verified:** an actual browser load, or any real
drag-and-drop / click-through of the new board — next session (or the
person, locally) should confirm before trusting this beyond "parses
cleanly and the hook order is provably stable by inspection." No
backend tests were added this session since no backend code changed —
Part 1/2/4's existing test coverage (listed above) already covers
everything the new frontend calls into.

**Still open:**
- Real browser verification of `DiscoveriesBoard` — drag each direction
  (Needs Triage → Job Postings, Needs Triage → Applications), click
  each equivalent, confirm the resolver modal's search box actually
  filters and that "Create new application" correctly hands off to
  `DiscoveryReviewModal` with the discovery still in scope.
- A real `pytest tests/` run against this session's (unchanged) backend
  to reconfirm the 250ish tests mentioned in Parts 1/2/4 above are
  still green — nothing here should have touched them, but that's an
  assumption pending a real run, same caveat as every prior sandboxed
  session.
- Per the spec's own scope note (Part 4): a cold job-alert digest for a
  company with *no* existing tracked item still won't surface at all —
  `search_unmatched_messages` only scans term-matched mail. Spec
  explicitly deferred this to a hypothetical "Part 5"; still deferred.

**Session 15 addendum**: the first cut of the Email Sync page (below) shipped
with a real bug — the user hit a blank screen with React's minified error
#310 ("Rendered more hooks than during the previous render") as soon as
the app loaded. Cause: the new `emailCount` `useState`/`useEffect` pair
in `App()` was placed *after* the component's early-return gates
(`if (!statusInfo) return ...`, `if (!statusInfo.workspace) return ...`,
etc.) instead of alongside App's other unconditional hooks above them.
On the very first render `statusInfo` is null, so React takes one of
those early returns and never reaches — never registers — that hook
pair; once `statusInfo` loads a moment later, the same component
instance suddenly calls two more hooks than it did last render, which
breaks React's hook-order bookkeeping and crashes. Classic Rules-of-Hooks
violation, and one the earlier Babel-syntax check (session 15's first
pass) could never have caught — a transpile check only proves the code
parses, not that it obeys React's runtime rules; there's no static
analysis in this sandbox (no `eslint-plugin-react-hooks`) that would
have flagged it either. **Fixed** by moving both hooks up next to
`App`'s other `useState`/`useEffect` calls, before any early return.
Re-verified: full Babel transpile still clean, and manually walked every
hook call in `App()` and `EmailHubPage()` in source order confirming
none sit after a conditional return. **Still not verified**: an actual
browser load — next session (or the user, locally) should confirm the
Email Sync page renders before trusting this beyond "transpiles and the
hook order is provably stable by inspection." Lesson for future
sessions: any new top-level hook added to `App()` (or any component with
early-return gates) goes above every early return, full stop — don't
rely on "it's just a useEffect, it'll be fine."

**Session 15** answered a UX complaint about session 14's discovery
queue and did a real redesign in response, not just a suggestion. The
complaint (from screenshots + description): with 66 pending discoveries,
`DiscoveriesSection` was a small collapsible panel *inside* the Settings
modal, so working through a big batch meant scrolling a cramped column
with the rest of the blurred, inert app behind it, no way to act on more
than one row at a time, and toasts (e.g. "No missing email PDFs found.")
rendering hard to read against the blur. **Fix**: pulled
`AccountsSection` + `DiscoveriesSection`'s functionality out of
`SettingsModal` entirely and into a new top-level page, `EmailHubPage`,
wired to a new "Email Sync" item in the sidebar `Rail` (own icon, own
badge showing the pending-discovery count, own command-palette entry
`nav-email`) — same tier as Pipeline/Insights/Manage, not a modal, so no
blur and no cramped column. Settings still has a small pointer
("Connected accounts and discoveries have moved") with a button that
navigates there and closes the modal, for anyone who goes looking in the
old spot. Added the multi-select the user explicitly asked for: every
discovery row got a checkbox, plus a toolbar with "Select all",
"Dismiss selected" (loops `POST /discoveries/{id}/dismiss` sequentially,
same one-at-a-time reasoning the codebase already used for `syncAll`/
`connectAllPicked` against the AppleScript bridge), and "Attach selected"
(only counts/acts on selected rows that are `ambiguous` *and* already
have a candidate application picked in their dropdown — a clean-match
row still needs the Review form since company/role/status are all
editable there, so bulk-accepting those without a look wasn't offered).
No backend changes were needed or made — every action the new page takes
calls an endpoint that already existed (`/api/accounts`, `/api/accounts/
{id}/sync`, `/api/accounts/mail-app/available`, `/api/accounts/mail-app/
connect`, `/api/accounts/{id}/discover`, `/api/discoveries`, `/api/
discoveries/{id}/dismiss`, `/api/discoveries/{id}/attach`, `/api/
discoveries/dismiss-sender`, `/api/discoveries/backfill-email-pdfs`);
this was purely a frontend information-architecture change. The old
`AccountsSection`/`DiscoveriesSection` function bodies were deleted
(not left as dead code) once their logic was folded into
`EmailHubPage`; `DiscoveryReviewModal` (the per-row "confirm and create"
form) was kept as-is and is still used from the new page.

Also investigated, but did **not** change: the user's question about
whether an account like `devmaddock@outlook.com` — specifically the
`.com` — could be "interfering with the syncing/now button." Nothing in
`mail_app_store.py` or the frontend treats a period in an email/domain
specially, and the "Sync now" button's disabled state (`disabled={syncing
=== a.id}` in what's now `EmailHubPage`) has no dependency on the
account's address at all. The one place a domain like `outlook.com`
does matter is `guess_company_from_email()`, which intentionally skips
`_GENERIC_SENDER_DOMAINS` (gmail/outlook/hotmail/icloud/yahoo, plus the
ATS domains) so it never guesses "Outlook" as an employer — but that
only affects the *guessed company* shown in a discovery row (and only
when `outlook.com` is the sender of an incoming application email, not
the connected account itself), never the Sync button. **Still
unverified**: none of the screenshots provided this session actually
showed the accounts list (all four discovery-queue screenshots + the
two pipeline screenshots + the command-palette screenshot), so if
there's a real visual bug (e.g. a long address overlapping the button)
it wasn't visible in what was shared — next session should get an
actual screenshot of the "Connected accounts" cards on the new Email
Sync page if the issue persists there.

**Verification this session**: no `uvicorn`/browser available in this
sandbox either (same limitation as sessions 12–14), but this sandbox
*did* have `npm` access this time — installed `@babel/core` +
`@babel/preset-react` and ran a real `transformSync` over the entire
extracted `<script type="text/babel">` block both before and after the
edits (not just brace-balance counting like session 14 had to fall back
on) — clean both times. Full existing `pytest` suite (242 tests) also
re-run against the untouched backend and still 242/242 passing. **Not**
verified: actually opening the app in a real browser and clicking
through the new Email Sync page, or the bulk-select flow against a real
Mail.app account with a large pending queue — next session (or the user,
locally) should do that before trusting this beyond "transpiles cleanly
and the backend it calls is unit-tested."

**Session 14** built the "discover new applications from inbox" feature
approved at the end of session 13's chat, then rebuilt it a second time
from scratch after discovering the zip the user uploaded to a *new* chat
session didn't contain the first build — that work lived only in the
now-closed sandbox from the session that built it and was never
persisted to a file. Lesson for future sessions: a chat session ending
(context limit, "out of free messages", etc.) does NOT mean the code
survives anywhere except a zip actually downloaded by the user — always
zip and hand off before a session might end, not after. The feature
itself, as rebuilt: `mail_app_store.py` gets `search_unmatched_messages()`
(scans one account's inbox via a single `whose`-clause Apple Event for
ATS-looking subject phrases / sender domains — `_ATS_SUBJECT_PHRASES`,
`_ATS_SENDER_DOMAINS` — then excludes anything already matching a
tracked item's company/role terms in Python) and
`guess_company_from_email()` (best-effort company-name guess from the
subject or sender domain, skipping known ATS/webmail domains so it never
guesses "Myworkday" or "Gmail" as an employer). The old inline
mailbox-resolution AppleScript inside `search_messages()` was factored
out into a shared `_resolve_mailbox_script()` helper so both functions
use the same two-step resolve-then-fallback-scan logic from session 13's
fix, rather than risking two copies drifting apart. `overrides_store.py`
gets a `discovered_matches` table (account_id, message_id, subject,
sender, received_at, guessed_company, status) plus
`add_discovered_match`/`list_pending_discoveries`/`get_discovery`/
`set_discovery_status` — `add_discovered_match` only ever INSERTs
(never UPDATEs), which is what makes an accepted/dismissed discovery
never resurface on a later scan. `api.py` gets four new routes:
`POST /api/accounts/{id}/discover` (runs the scan, files pending rows,
returns the queue), `GET /api/discoveries`, `POST
/api/discoveries/{id}/dismiss`, and `POST /api/discoveries/{id}/accept`
(Form: company/role_label/status — creates a real
`Applications/<company>/` folder via the same `create_application_folder`
+ rebuild path `POST /api/applications/new` uses, then links the
original email to the new item as a normal `account_match`, so an
accepted discovery is in every way an ordinary tracker item afterward,
not a special type). Frontend: `DiscoveriesSection` (a "Possible new
applications" panel in Settings, next to `AccountsSection`, with a
"Check inbox now" button that calls `/discover` for every connected
account) and `DiscoveryReviewModal` (pre-filled company/role/status form,
same shape as `NewApplicationModal`, backed by the accept endpoint).

**Testing done this session**: `test_mail_app_store.py` — 24/24 passing
(15 pre-existing + 9 new: known-terms filtering, the ATS whose-clause
being used, empty-known-terms behavior, limit handling, malformed-line
skipping, and four `guess_company_from_email` cases). Manual end-to-end
script against a temp sqlite db verified `discovered_matches` CRUD,
specifically that an accepted/dismissed row never resurfaces even across
reopening the db connection. All three edited/new Python files
(`api.py`, `mail_app_store.py`, `overrides_store.py`) pass `ast.parse`.
The two new frontend components were balance-checked in isolation
(parens/braces/brackets net to zero, every JSX tag's open/close count
matches) since this sandbox has no network access to install
`babel`/`fastapi`/any JS parser to do a real transpile or import check —
same limitation session 13 hit with pytest. **Still unverified**:
running the actual app (`uvicorn` import, real Babel transpile of
`index.html`, and of course the AppleScript itself against real
Mail.app/real inboxes) — none of that was possible in this sandbox.
Next session (or the user, locally) should run the real pytest suite,
actually load Settings in a browser, and click "Check inbox now" for
real before trusting this beyond what's described above.

**Open design questions, still unresolved** (carried over from session
13's chat, not decided, just implemented one way): how aggressive the
ATS heuristic should be (currently a fixed phrase/domain list, not
tunable); discovery is manual-only ("Check inbox now"), not run
automatically alongside "Sync now" — could be revisited once real usage
shows whether that's annoying; company-guessing from a subject/sender is
inherently fuzzy and is always shown as an editable field, never
auto-applied silently.

**Session 13** found that session 12's Bug 2 "fix" (`inbox of acct`)
was not actually fixed — the user hit the *same* `-1728 Can't get
inbox of account id ...` error again, on two different real accounts
(`willzaeagle@gmail.com` and `willzaeagle@hotmail.com`), confirmed via
screenshots of the Settings diagnostics panel. Session 12 could not
verify this against a real Mac and said so explicitly; the mock-based
regression test only checked that `inbox of acct` appeared in the
generated script text, which it did — that's not the same as
Mail.app actually resolving it to a mailbox object. **Fixed** by no
longer betting on a single property or name: `search_messages()` now
tries `inbox of acct` wrapped in `try`, and on failure falls back to
scanning `every mailbox of acct` for a same-named match (case
handled by AppleScript's default case-insensitive comparison); if
neither resolves, it raises a distinct `JOBTRACKER_NO_INBOX` marker
that `_run_applescript()` turns into a new `MailAppNoInboxError`
(subclass of `MailAppError`) with an actionable message — "open
Mail.app, click this account, let it finish loading" — instead of a
bare AppleScript error code. `sync_account()` in `api.py` needed no
changes: it already treats any non-`MailAppPermissionError`
`MailAppError` as a one-off failed attempt rather than `blocked`, so
the new exception type falls into that path automatically. Added 3
regression tests (fallback-scan present in script, the new exception
type raised on the marker, custom-mailbox fallback) plus updated 2
existing tests that had encoded the now-wrong assumption that `inbox
of acct` alone was sufficient. Full suite: 15/15 in
`test_mail_app_store.py` (ran directly, no pytest binary available in
this sandbox — see note in §27). **Still unverified on a real Mac**:
whether the fallback scan actually finds the Inbox for these two
accounts, and whether `MailAppNoInboxError`'s diagnosis (mailboxes not
yet loaded) is the real cause versus something else entirely. Full
detail in §27.

**Session 12** put session 11's Mail.app rewrite in front of the
user's real Mac, real 718-document tracker, and real 6-account Mail.app
setup for the first time (via a screen recording). Found and fixed two
genuine AppleScript bugs (a timeout against a real ~700-message Gmail
inbox from an O(n) Apple-Event-per-message loop, and a real `-1728`
error from an invalid `mailbox "INBOX"` reference) plus one mislabeling
bug (both got reported as "Automation permission needed" when neither
was a permission problem). See §26 for full detail and what's still
unverified.

**Session 11** rebuilt the connected-accounts feature twice in one
session. First it patched the OAuth-only-domain gap in the old
`accounts_store.py` scaffold (Gmail/Outlook/Hotmail/Live domains now
get rejected up front, both in the frontend form and in
`connect_imap()`, instead of hitting a confusing 501 from the unbuilt
IMAP stub). Then, once the user pointed out every account they care
about (5 total: 2 Outlook, hotmail, an unlabeled Outlook alias, and
Google) is already configured and logged in under macOS's own
Internet Accounts, and confirmed the app is macOS-only, that whole
OAuth/IMAP scaffold was thrown out in favor of driving Mail.app
directly over AppleScript. `accounts_store.py` is gone;
`mail_app_store.py` replaces it. See §25 for the full design and
testing detail.
...
## 25. Session 11 — Connected accounts rebuilt on AppleScript/Mail.app, replacing the OAuth/IMAP scaffold

**Starting point:** `accounts_store.py` was a documented scaffold —
Gmail/Outlook OAuth stubbed pending real client credentials, iCloud/
generic IMAP stubbed pending an `imaplib` implementation, tokens
destined for the OS keychain via `keyring`. The user hit the IMAP form
directly with an Outlook address and got a 501 from the unbuilt stub,
after typing `login.microsoftonline.com` (Microsoft's OAuth consent
page URL) into the "IMAP host" field — a reasonable mistake, since
that field doesn't explain it can't ever work for Outlook.

**First fix (superseded below, kept for the record):** confirmed via
web search that Microsoft killed IMAP/POP basic auth for Outlook.com/
Exchange in October 2023 and SMTP basic auth in March 2026 — a bare
password can never authenticate an Outlook/Hotmail/Live/Gmail account
over plain IMAP anymore, regardless of implementation effort. Added:
- Frontend: `looksLikeOAuthOnlyDomain()` in the IMAP form warns inline
  before submit for those domains.
- Backend: `connect_imap()` rejected the same domains with a clear
  `ValueError`, so the guard holds even off the UI path.

**What changed the direction entirely:** the user pointed out macOS's
own Internet Accounts pane (System Settings) already had all 5 of
their accounts logged in and syncing — `devaios12@outlook.com`,
`devmaddock@outlook.com`, an account named "hotmail" (address
`stumping123@outlook.com`), `stumping123@outlook.com` directly, and a
Google account — confirmed via screenshot. Mail.app already holds
valid, refreshed tokens for all of them. The user confirmed the app is
macOS-only, so there's no Windows/Linux fallback to preserve.

**Decision: replace the OAuth/IMAP scaffold with AppleScript against
Mail.app**, not add it alongside. Rationale: no app registration with
Google/Microsoft, no client_id/secret, no "unverified app" screen or
admin-consent wall, no credential storage of any kind (Mail.app owns
that) — one macOS Automation permission prompt covers every account
instead of five separate OAuth logins.

**Implementation:**
- `_app/accounts_store.py` deleted entirely.
- `_app/mail_app_store.py` (new): `_run_applescript()` wraps
  `osascript` via `subprocess.run`, with dedicated error text for
  "osascript missing" (non-Mac), a timeout, and a denied Automation
  permission (detected via `-1743`/"not allowed" in stderr) that tells
  the user exactly where to fix it (System Settings → Privacy &
  Security → Automation). `list_mail_app_accounts()` enumerates every
  Mail.app account via `tell application "Mail" ... every account`.
  `search_messages(account_name, terms)` searches one account's INBOX
  for messages whose subject or sender contains any of the given terms
  (company name / role title), returning message id (the RFC822
  Message-ID header, stable across re-syncs — not Mail.app's
  session-local numeric `id`), subject, sender, and received date.
- `_app/overrides_store.py`: `accounts` table gained an `account_name`
  column (migrated in `_migrate()` for pre-existing `overrides.db`
  files) — this is Mail.app's own account name, which
  `search_messages()` needs to query by and which can differ from the
  displayed `email` (e.g. the "hotmail" account above). `status`
  vocabulary changed from `connected/needs_reauth/disconnected` to
  `connected/blocked/disconnected` — `blocked` means the last sync hit
  a denied Automation permission, not an expired token (there is no
  such thing anymore).
- `_app/api.py`: removed the OAuth `connect`/`callback` routes and the
  `imap-connect` route. Added `GET /api/accounts/mail-app/available`
  (lists Mail.app accounts not yet connected, by `account_name`) and
  `POST /api/accounts/mail-app/connect` (no handshake — just records
  the row; Mail.app already did the login). `disconnect_account` no
  longer calls anything keychain-related. `sync_account` went from a
  501 placeholder to a real implementation: loads every non-archived
  application item for the current workspace
  (`db.load_applications()`), builds search terms from
  `effective_company` and `role_label`, calls
  `mail_app_store.search_messages()` per item, and records new hits
  via `ov.add_account_match()` (de-duped by message id, so re-sync is
  always safe) before `ov.record_sync()`. A `MailAppError` mid-sync
  marks the account `blocked` and surfaces the message as a 400
  instead of silently failing.
- `_app/frontend/index.html`'s `AccountsSection`: replaced the four
  provider buttons (Gmail/Outlook/iCloud/Other-IMAP) and their forms
  with a single "+ Connect an account" button that opens a picker
  (`GET /api/accounts/mail-app/available`) and a "Sync now" button per
  connected account. `looksLikeOAuthOnlyDomain()` and the IMAP form
  are gone from the frontend too, since there's no IMAP path left to
  guard.
- `_app/requirements.txt`: dropped `keyring` — nothing left to store
  in a keychain.

**Testing:** `tests/test_mail_app_store.py` (10 tests) mocks
`subprocess.run` to cover account-list parsing, message-search
parsing, the synthetic-id fallback when a message has no `message id`
header, and all three `MailAppError` paths (missing osascript, denied
permission, other failure) without needing a real Mac. `test_accounts_api.py`
(9 tests) exercises the full API surface — available/connect/list/
disconnect, and sync's matching + de-dupe + blocked-on-error behavior —
against `sample_root`'s one seeded application (Acme Co / Backend
Engineer), with `mail_app_store` mocked at the `api_module.mailapp`
boundary. Full suite: 169/169 passing (was 150 before this session).
Claude has not staged, committed, or pushed anything, per §6.

**Not done / open:**
- No real macOS testing yet — everything above is exercised through
  mocks in this Linux sandbox. The user should run this for real
  (connect one of the 5 accounts, click Sync, confirm the Automation
  permission prompt appears and works) before trusting it end to end.
- `search_messages()` only searches each account's `INBOX` mailbox.
  Company confirmation/rejection emails that land in a different
  folder (a filter rule, "Archive", a custom mailbox) won't be found.
  Worth a follow-up if that turns out to matter in practice.
- No background/automatic sync — sync is manual-trigger only via the
  "Sync now" button, matching the existing `/api/accounts/{id}/sync`
  route's original design intent.
- README.md doesn't yet have a "Connected accounts" section
  documenting any of this for a future reader — the original
  `accounts_store.py` docstring referenced one that was never actually
  written.

## 26. Session 12 — real-Mac testing found and fixed two live bugs in the AppleScript sync path

The user tested §25's Mail.app rewrite against their real, live tracker
(the actual 110-application/718-document `JobTracker — working-db (2)`
workspace, not `sample_root`) and a real Mail.app with 6 configured
accounts. Two genuine bugs surfaced, both fixed this session, plus one
mislabeling bug in how errors were reported:

**Bug 1 — sync timed out against a real ~700-message Gmail inbox.**
`search_messages()`'s original implementation did
`repeat with msg in (messages of targetBox)` and tested each message's
subject/sender inside the loop — a well-known Mail.app AppleScript
performance trap, since each property access is its own Apple Event
round-trip. Against `willzaeagle@gmail.com` this exceeded the 60s
timeout and surfaced as "Mail.app didn't respond in time." **Fixed**
by switching to `messages of targetBox whose (subject contains "X" or
sender contains "X") or (...)`, which Mail.app evaluates as a single
Apple Event internally — only the (small) matched set gets iterated
afterward to extract properties.

**Bug 2 — real `-1728 Can't get object` error on `mailbox "INBOX" of
acct`.** That reference looks right but isn't valid for every
provider's account object (confirmed against the same Gmail account).
**Fixed** by using Mail.app's built-in `inbox of acct` property for
the default mailbox instead — reliable across providers — while a
caller-specified non-default mailbox name still goes through
`mailbox NAME of acct`, since there's no built-in property for
arbitrary folders.

**Bug 3 (mislabeling, not AppleScript) — both of the above got
reported to the user as "Automation permission needed," which was
wrong and sent them to check System Settings → Privacy & Security →
Automation for nothing (confirmed via screenshot: PyCharm → Mail.app
was already toggled on the whole time).** Root cause: `sync_account()`
in `api.py` marked any `MailAppError` as `status: "blocked"`, and the
frontend hardcoded "Automation permission needed" for that status
regardless of the real cause. **Fixed** by splitting
`MailAppPermissionError(MailAppError)` out as its own type, raised
only when `_run_applescript()` actually detects a denied-permission
signature (`-1743`/"not allowed"/"not authorized" in stderr).
`sync_account()` now only marks an account `blocked` for that specific
exception; a timeout, a mailbox-lookup failure, or anything else is
surfaced as a one-off failed attempt (the real error message, via the
existing `error` banner) without touching the account's persistent
status.

**Testing:** `test_mail_app_store.py` gained three regression tests
asserting the generated AppleScript actually contains
`messages of targetBox whose` (not the old manual repeat) and
`inbox of acct` (not `mailbox "INBOX"`), plus updated the permission
test to check for the new `MailAppPermissionError` subclass.
`test_accounts_api.py`'s blocked-status test now uses
`MailAppPermissionError` and gained a sibling test confirming a
generic `MailAppError` (e.g. a timeout) leaves status as `connected`
rather than `blocked`. Full suite: **173/173 passing** (was 169).
Claude has not staged, committed, or pushed anything, per §6.

**Still open, unverified on a real Mac after these fixes:**
- Whether the `whose`-filter search is actually fast enough in
  practice against the same live Gmail account — the user hit the
  timeout before either fix landed, so re-testing sync against
  `willzaeagle@gmail.com` (and ideally the other 5 accounts) is the
  next concrete step.
- Whether `inbox of acct` resolves correctly for all 6 of the user's
  real accounts (2 Outlook work-style, hotmail, a second Outlook
  alias, Gmail, iCloud) — only Gmail's failure mode is confirmed fixed
  so far.
- The video capture also showed the user opening a separate window
  with what looked like a real Amazon application-confirmation email
  mid-session — unclear if that was just manual verification of what
  a real match should look like, or a separate thread; nothing in it
  changed anything about this session's fixes.

## 27. Session 13 — the "inbox of acct" fix from §26 wasn't real; Inbox resolution is now two-strategy with a distinct not-yet-loaded error

Picking up right where §26 left off: the user re-tested sync against
`willzaeagle@gmail.com` (the account §26 flagged as the best test of
the `inbox of acct` fix) and hit the *identical* `-1728 Can't get
inbox of account id "6A420779-..."` error — screenshots confirm it,
timestamp-stamped the same session as the connect/sync UI. Worse,
re-testing against a second, unrelated account (`willzaeagle@hotmail.com`)
hit the same `-1728` with a different account id
(`19C74383-9E76-4F33-ADF2-F3C7539BE5A1`). So this isn't a Gmail-specific
quirk — the built-in `inbox of acct` property that §26 introduced (and
verified only via a mock asserting the *string* `"inbox of acct"*`
appeared in the generated script, not that it actually resolves)
is not reliably backed by a real mailbox object, at least not for
these two accounts at this point in their lifecycle.

**Root cause, best guess (unverified on a real Mac):** most likely
Mail.app hadn't finished populating these accounts' local mailbox
lists yet — both were connected in the app moments before the sync
attempt in the recording. Whether `inbox of acct` would eventually
start working for the same accounts once Mail.app finishes an initial
sync is untested. Since neither this repo nor this sandbox has a real
Mac or `osascript`, this session couldn't confirm the mechanism, only
make the failure mode survivable and diagnosable.

**Fix:** `search_messages()` in `mail_app_store.py` no longer commits
to one lookup strategy for the target mailbox. The generated
AppleScript now:
1. Tries `inbox of acct` (or, for a caller-specified non-default
   mailbox, `mailbox "NAME" of acct`), wrapped in a bare `try...end
   try` so a failure leaves `targetBox` as `missing value` instead of
   aborting the script.
2. If that didn't resolve anything, falls back to `repeat`-scanning
   `every mailbox of acct` for one whose `name` matches ("INBOX", or
   the caller's mailbox name) — AppleScript's default string
   comparison is already case-insensitive, so no explicit
   case-folding is needed here.
3. If *neither* resolves a mailbox, the script raises an AppleScript
   `error` with a `JOBTRACKER_NO_INBOX:` prefix instead of letting a
   raw `-1728` propagate. `_run_applescript()` now checks stderr for
   that marker and raises a new `MailAppNoInboxError(MailAppError)`
   with an actionable message (open Mail.app, click the account, let
   it finish loading, retry) rather than surfacing the bare AppleScript
   error text.

`api.py`'s `sync_account()` needed **no changes** — it already
catches `MailAppPermissionError` specifically and falls through to a
generic `except MailAppError` for everything else, marking the
account `connected` (not `blocked`) and surfacing the message as a
one-off failed attempt. `MailAppNoInboxError` is a `MailAppError`
subclass that isn't `MailAppPermissionError`, so it lands in that
generic branch automatically — exactly the right behavior, since a
not-yet-loaded account is retryable, not a permission problem.

**Testing:** `test_mail_app_store.py` — renamed the now-inaccurate
"uses_inbox_property" test to `..._tries_inbox_property_first` (checks
`try` wraps it), added
`test_search_messages_default_mailbox_falls_back_to_scanning_mailbox_list`
(asserts `every mailbox of acct` and the name-match line are present
in the generated script), added
`test_search_messages_raises_no_inbox_error_when_nothing_resolves`
(feeds a `JOBTRACKER_NO_INBOX` stderr through `_run_applescript` and
asserts the specific exception type, mirroring the existing
denied-permission test's shape), and renamed/extended the custom-
mailbox test to also assert its fallback line is present. Ran all
tests directly with `python3` (imported the test module and called
every `test_*` function) since this sandbox has no network access to
install `pytest` — 15/15 passing in `test_mail_app_store.py`. Did not
re-run `test_accounts_api.py` or the full suite total from §26 (was
173); nothing in this change touches `api.py` or the accounts-API
surface, so no reason to expect regressions there, but that's an
assumption, not a re-verified number — worth a real `pytest tests/`
run next session.

**Still open, unverified on a real Mac after this fix:**
- Whether the fallback scan (`every mailbox of acct` by name) actually
  succeeds where `inbox of acct` failed, for both
  `willzaeagle@gmail.com` and `willzaeagle@hotmail.com`. This is the
  single most important thing to re-test — everything else in this
  session is reasoning from the error text and Mail.app's documented
  (but apparently not fully reliable) scripting dictionary, not
  confirmed behavior.
- Whether `MailAppNoInboxError`'s diagnosis (mailboxes not yet loaded)
  is actually the real cause, or whether these two accounts have some
  other persistent issue (e.g. disabled "Mail" toggle in Internet
  Accounts despite being visible to `list_mail_app_accounts()`, or a
  provider-specific mailbox-naming quirk neither `"INBOX"` nor a
  loaded-state theory explains). If the fallback scan *also* fails and
  the user sees the new `MailAppNoInboxError` message, that's the next
  concrete signal to act on — and worth asking the user to also check
  what mailbox names Mail.app's own UI shows for these two accounts.
- The other 4 of the user's 6 real accounts (2 Outlook work-style
  addresses, a second Outlook alias, iCloud) still haven't been
  sync-tested against any version of this code at all.

## 28. Session 14 — company-only sync matches now disambiguated instead of silently misattributed

**IMPORTANT: this session ran in an environment with no network access,
so `pip install` and the real `pytest` run were both unavailable.**
Everything below the schema/matching-logic changes was verified by
directly exercising the pure-Python functions (`mail_app_store.
search_messages()`, `overrides_store`'s schema/migration/round-trip) in
isolation, and by syntax-checking the edited Python files and running
the frontend's `<script type="text/babel">` block through `esbuild`'s
JSX parser (catches syntax errors, not runtime/logic bugs). **The full
`pytest` suite, including all new tests added this session, has NOT
been run.** Before relying on this, run:
```
pip install -r _app/requirements.txt -r requirements-dev.txt
pytest
```
and confirm all tests (237 pre-existing + the new ones in
`test_accounts_api.py` and `test_discoveries.py`) pass.

**Root cause investigated:** the user reported a real-world
misattribution — a genuinely generic "Notice from Adams County"
rejection email (confirmed from the user's own uploaded PDF: subject
mentions only "Adams County Government" broadly, nothing about which
specific role) whole-word-matches ANY open item whose company is
"Adams County", with no way to tell which one it's actually about once
that company has more than one open application and the role label on
at least one of them is too generic to search on (e.g. "IT", dropped
by `is_usable_match_term()`). `sync_account()` searched each item
independently and trusted a company-only text hit unconditionally —
there was no cross-item check for "does this company have other open
items this could just as easily belong to?"

**Fix:**
- `mail_app_store.py`: `search_messages()` hits now carry a
  `company_only: bool` field -- true when a company term matched but
  no role term did (either no usable role term was ever passed in, or
  one was passed but didn't appear in the message). False for a
  thread-id match (deterministic, no ambiguity) and for any hit where
  a role term also matched.
- `overrides_store.py`: `discovered_matches` gained `match_kind`
  ('unmatched' | 'ambiguous', default 'unmatched' for backwards
  compat) and `candidate_item_keys` (JSON list) columns, with a
  migration for existing overrides.db files. `add_discovered_match()`
  accepts both as optional kwargs. `list_pending_discoveries()` /
  `get_discovery()` now decode `candidate_item_keys` back into a
  Python list.
- `api.py`:
  - `sync_account()` groups all currently-open items by normalized
    `effective_company` up front. A hit with `company_only=True`
    against a company with 2+ open items is routed into the
    discoveries queue as `match_kind='ambiguous'` (with the sibling
    item_keys as candidates) instead of being auto-attached via
    `add_account_match()`. Response now also returns
    `ambiguous_matches` (count) alongside the existing `new_matches`.
  - New `POST /api/discoveries/{id}/attach` (item_key form field):
    lets the user pick which existing item an ambiguous discovery
    actually belongs to. Records it as an ordinary `account_match` +
    thread identifier on that item, marks the discovery 'accepted'.
    Validates `item_key` against currently-open items (400 if
    unknown), 409 if the discovery was already decided.
  - `_discoveries_with_account_email()` now also takes an optional
    `jt_conn` to build a human-readable label ("Company — Role") per
    candidate item_key for 'ambiguous' rows, via
    `db.load_applications()`. All call sites updated to pass it.
- `frontend/index.html`'s `DiscoveriesSection`: rows with
  `match_kind === "ambiguous"` now render a "Which application?" badge,
  an explanatory line, a `<select>` of the candidate items, and an
  "Attach" button (calling the new endpoint) in place of the ordinary
  "Review" (create-new-application) button. "View email" and "Dismiss"
  are unchanged and still work identically for both kinds of rows.

**Update (Session 15): full suite now run and green.** The user ran
`pytest` on their Mac and got 238/242 passing — the 4 failures were
all the same root cause: `sync_account()`'s response dict gained a new
`ambiguous_matches` key (alongside the pre-existing `ok`/`new_matches`
keys), which broke pre-existing tests doing exact `==` comparisons
against the old two-key shape. Fixed by widening those 4 assertions to
include `"ambiguous_matches": 0` (the correct value for each of those
scenarios, which all involve a single, unambiguous match):
`test_accounts_api.py::test_sync_matches_against_the_one_open_application`,
`test_accounts_api.py::test_sync_persists_thread_message_ids_from_a_confirmed_hit`,
`test_accounts_api.py::test_sync_hit_missing_thread_message_ids_key_does_not_break`,
`test_email_pdf.py::test_backfill_saves_pdf_for_a_match_that_predates_the_feature`.
No production code changed for this fix — only the 4 test assertions.
Re-ran the full suite in this session's sandbox (network access was
available here, unlike Session 14) with a real `pip install` +
`pytest`: **242/242 passed.**

**Tests added (now run — see update above):**
- `test_accounts_api.py::test_sync_routes_company_only_hit_to_ambiguous_queue_when_company_has_two_open_items`
  — reproduces the Adams County shape directly: two open items under
  one company, one role generic enough to be dropped, a company-only
  hit lands in the discoveries queue instead of an `account_match`.
- `test_accounts_api.py::test_sync_still_auto_attaches_when_role_term_also_confirms`
  — companion regression check: the SAME multi-item company still
  auto-attaches normally when a hit's role term also matches.
- `test_discoveries.py::test_attach_ambiguous_discovery_records_match_on_chosen_item`,
  `test_attach_rejects_unknown_item_key`,
  `test_attach_already_decided_discovery_is_409` — cover the new
  `/attach` endpoint directly.

**Also investigated and found NOT to be a bug:** the user's pasted
"was in: Adams County / IT rejected ... instead of: Public Health
Department Commerce City / Environmental Health Specialist applied
..." comparison. Traced this in the uploaded jobtracker.db/overrides.db
pair to two genuinely distinct, correctly-separated items (item 4:
`Adams County / IT`, unrelated Feb-2026 rejection; item 86: `Public
Health Department Commerce City / Environmental Health Specialist`,
correctly auto-detected an Aug-2026 applied date from its own
`application_confirmation` document) — both items already have
different `source_relpath` folders and different document
`content_hash`es despite sharing a coincidentally-identical filename
("Application received by Adams County.pdf"). Nothing to fix there;
it was the forward-looking version of this exact ambiguity risk (would
a FUTURE company-wide email misfile itself into the wrong one of these
two?) that turned out to be real and is what this session's fix
addresses.

**Still open:**
- ~~The full test suite has not been run in this session~~ — done in
  Session 15, 242/242 passing.
- No real-Mac / real-Mail.app verification this session (matches the
  network-restricted environment, not a design choice).
- The frontend's ambiguous-picker UI has not been visually verified in
  a browser (esbuild's JSX parse only catches syntax errors, not
  rendering/runtime issues) — worth a manual click-through once
  running locally.
- Consider also flagging `guessed_company` mismatches (e.g. "Adams
  County" vs "Public Health Department Commerce City" in the user's
  own example) via `company_aliases` merges as a complementary,
  longer-term fix — this session's change only handles the
  same-literal-company case.

## 29. Session — Email Sync board v2, Part 5 (job-posting link + date enrichment)

**Note on continuity:** this repo's code already contains the 3-column
`DiscoveriesBoard` (Job Postings / Needs Triage / Applications) plus the
`kind`/`mark-posting` backend from `discoveries-kanban-spec.md` Part 3,
but this HANDOFF.md's last entry (§28) predates that work — it was
built in a session whose notes never made it back into this file. Flagging
here so a future session doesn't assume the board doesn't exist just
because it isn't narrated above. Nothing in that gap needed fixing; this
session started from the board already being live and used against a
real inbox (66+ discoveries).

**What prompted this:** using the board against a real inbox surfaced
that Job Postings cards were too thin to be useful — no link to the
actual posting, just a raw AppleScript-style timestamp
("Wednesday, October 15, 2025 at 9:22:46 AM"). Root cause: discovery
rows only ever stored `subject`/`sender`/`received_at`/`guessed_company`
— nothing from the email body — so there was nothing to link to even
though `extract.extract_urls()` already existed and could have found one.

This is Part 5 of a new spec, `discoveries-board-v2-spec.md` (Parts
5-8: link/date enrichment, archive, stats bar, filtering) — written this
session after a walkthrough of the live board on a real inbox. Parts 6-8
are designed but not yet built.

**Backend:**
- `overrides_store.py`: `discovered_matches` gained a `posting_url TEXT`
  column (migration included), and a `set_discovery_posting_url()`
  setter. NULL until a posting has been previewed at least once; stays
  NULL forever if nothing recognizable was found in the body (no fake
  links are ever stored).
- `mail_app_store.py`: new `guess_posting_url(body)` — runs
  `extract.extract_urls()` against the body and returns the first URL
  whose domain matches a known job-board/ATS list (Greenhouse, Lever,
  LinkedIn jobs, Indeed, Workday, iCIMS, SmartRecruiters, Taleo,
  Jobvite, Ashby, BambooHR, ZipRecruiter, Workable, Breezy), explicitly
  skipping unsubscribe/tracking/preferences links even on those same
  domains. Returns `None` rather than guessing when nothing qualifies.
- `api.py`'s `preview_discovery()` (`GET /api/discoveries/{id}/preview`)
  now also returns `posting_url`, computing and persisting it lazily on
  the same trip that already fetches the body — only for `kind='posting'`
  rows that don't already have one. **Response shape changed** from
  `{"body": ...}` to `{"body": ..., "posting_url": ...}` — 3 pre-existing
  tests in `test_discoveries.py` asserted the old two-key shape via exact
  `==` and needed widening (same failure mode as §28's `ambiguous_matches`
  addition).

**Frontend (`frontend/index.html`):**
- New `timeAgo(raw)` helper parses Mail.app's verbose date string (strips
  the weekday and "at" so `new Date(...)` can parse it) into "3d ago" /
  "2mo ago" style labels; falls back to `null` (raw date still shown) if
  parsing fails.
- `DiscoveryCard`: Job Postings cards now show "posted 3d ago" instead of
  the raw timestamp, and an "Open posting ↗" link when `posting_url` is
  present — nothing shown when it isn't, per the "never fake a link"
  rule above.
- `toggleExpand()` in `EmailHubPage` now merges a returned `posting_url`
  into the `discoveries` list state, so the card picks it up without a
  full reload.

**Tests added:**
- `test_mail_app_store.py`: `test_guess_posting_url_picks_known_ats_link`,
  `test_guess_posting_url_skips_unsubscribe_even_on_a_known_domain`,
  `test_guess_posting_url_returns_none_when_nothing_recognizable`,
  `test_guess_posting_url_returns_none_for_empty_or_none_body`.
- `test_discoveries.py`:
  `test_preview_finds_and_persists_a_posting_url_for_a_posting_kind_discovery`
  (finds it, persists it, second call reuses the cached value even if
  the mail fetch itself fails that time),
  `test_preview_posting_kind_with_no_recognizable_link_leaves_posting_url_none`.

**Verified this session:** real `pip install` + `pytest` — **263/263
passing** (network was available in this sandbox). Frontend's
`<script type="text/babel">` block parsed cleanly through `esbuild`
(syntax-level check only, not a runtime/rendering check).

**Still open:**
- No real-browser click-through of the new "posted Xd ago" label or
  "Open posting" link — worth clicking through once running locally,
  especially to confirm the date parsing handles every timestamp shape
  Mail.app actually produces (only the one sample format from earlier
  sessions' screenshots was tested here).
- The job-board domain list in `guess_posting_url()` is a best guess at
  what's common — likely needs tuning against your actual inbox's real
  postings (e.g. if a lot of yours come through a specific company
  career-site domain rather than a shared ATS).
- Parts 6 (archive), 7 (stats bar), 8 (filtering) from
  `discoveries-board-v2-spec.md` are designed but not started.

## Part 5.5 — Adaptive sender classification + multi-listing digests

Implements `discoveries-sender-classification-and-digests-spec.md` in
full, against the real Haystack LinkedIn-digest email that motivated
it (confirmed: 6 listings in one email — Haystack, Torentify x2,
Piper, Ladders x2 — previously vanished entirely, not just miscategorized,
because `linkedin.com` is a mixed-signal sender domain and LinkedIn
renames the subject to the top-ranked listing, so it never contains an
ATS phrase).

**Fixed while in this code (per the spec):** the "Open posting ↗" link
on the postings card was still a plain `<a target="_blank">` instead
of this app's own `openExternalUrl()` helper — the one-line fix is
folded into this same pass.

**Part 5.5a — sender whitelist:**
- `overrides_store.py`: new `job_posting_senders` table (`sender TEXT
  PRIMARY KEY, added_at TEXT`), migration-safe (`CREATE TABLE IF NOT
  EXISTS`, so it applies to existing overrides.db files with no ALTER
  needed). New functions: `add_job_posting_sender`,
  `remove_job_posting_sender`, `list_job_posting_senders`.
- `mail_app_store.search_unmatched_messages()`: new
  `always_posting_senders: list[str] | None` parameter. Adds a fourth
  `whose`-branch (`sender is "..."` per entry, OR'd in) to the same
  single AppleScript scan, and the whitelisted branch bypasses BOTH the
  ATS-subject-phrase gate and the digest-subject exclusion entirely.
  Each returned hit now also reports `force_posting: bool`.
- `api.py`: `discover_new_applications()` fetches
  `ov.list_job_posting_senders()` before scanning and passes it through;
  classifies `kind="posting"` whenever `force_posting` is true, in
  addition to the existing `is_job_posting_style_subject()` check. New
  `POST /api/discoveries/sender-classification` (Form: `sender`) calls
  `ov.add_job_posting_sender()` and returns the refreshed pending queue
  (same response shape as `dismiss-sender`).
- Frontend: "Always treat this sender as postings" button on every
  discovery card with a sender (Job Postings or Needs Triage) — calls
  the new endpoint. Only affects future scans, doesn't relabel anything
  already queued.

**Part 5.5b — multi-listing digests:**
- `mail_app_store.py`: new `extract_posting_urls(body) -> list[str]`
  (same domain/exclusion lists as before, but collects every matching
  URL in body order instead of returning on the first hit, de-duped via
  `extract.extract_urls()`'s existing order-preserving dedupe).
  `guess_posting_url()` now delegates to it (`urls[0] if urls else
  None`) — kept for backward compat, no existing single-link callers
  needed to change.
- `overrides_store.py`: new `posting_urls TEXT` column on
  `discovered_matches` (JSON array, ALTER-TABLE migration for existing
  dbs). `posting_url` stays as-is and is kept in sync as
  `posting_urls[0]` by the new `set_discovery_posting_urls()`.
  `_decode_discovery()` now also JSON-decodes `posting_urls`.
- `api.py`: `/api/discoveries/{id}/preview` now also computes/persists
  `posting_urls` (via `extract_posting_urls`) on first open, same lazy-
  cache timing as the existing `posting_url` behavior, and returns both
  in the response.
- Frontend: `DiscoveryCard` renders every link in `posting_urls` as a
  compact inline list (`Listing 1 ↗ · Listing 2 ↗ · ...`) when there's
  more than one, falling back to the single `posting_url` link
  otherwise — per-listing company names weren't attempted (would need
  more digest-body parsing than this pass covers), so labels are
  numbered rather than guessed, per the spec's fallback rule. Added a
  text filter input above the Job Postings column (matches
  subject/company) — a plain `<input>` for now since Part 8's shared
  filter-pill UI hasn't landed yet; swap this out if/when it does, per
  the spec's "don't build two search UIs" note.

**Tests added:**
- `test_discoveries.py`: updated the 4 `/preview`-shape assertions to
  include `posting_urls`; added
  `test_preview_persists_every_listing_link_for_a_multi_listing_digest`
  (the real 3-link-subset regression case),
  `test_sender_classification_endpoint_whitelists_a_sender`,
  `test_sender_classification_makes_a_future_scan_force_posting_regardless_of_subject`
  (confirms the whitelist is actually threaded through to
  `search_unmatched_messages`'s call and produces `kind="posting"` for a
  subject with zero ATS-phrase signal).
- `test_mail_app_store.py`: `test_search_unmatched_messages_adds_a_
  whitelist_whose_branch_for_posting_senders`,
  `test_search_unmatched_messages_with_no_whitelist_omits_the_sender_
  is_branch`,
  `test_search_unmatched_messages_reports_force_posting_for_a_
  whitelisted_sender_hit` (the literal Haystack case: no ATS phrase in
  subject, whitelisted sender, `force_posting=True`),
  `test_search_unmatched_messages_non_whitelisted_hit_reports_force_
  posting_false`, `test_extract_posting_urls_returns_every_matching_
  link_in_body_order`, `test_extract_posting_urls_dedupes_while_
  preserving_order`, `test_extract_posting_urls_returns_empty_list_for_
  empty_or_none_body`, `test_extract_posting_urls_returns_empty_list_
  when_nothing_recognizable`,
  `test_guess_posting_url_still_returns_just_the_first_link_for_
  backward_compat`.
- `test_overrides_store.py`: `test_fresh_db_has_job_posting_senders_
  table_and_posting_urls_column`,
  `test_migration_adds_posting_urls_column_to_a_pre_5_5_db` (hand-built
  legacy schema, confirms the ALTER TABLE migration and that an
  existing row's `posting_url` survives untouched while `posting_urls`
  defaults to `[]`), `test_add_and_list_job_posting_senders` (including
  idempotent re-add), `test_remove_job_posting_sender`,
  `test_set_discovery_posting_urls_persists_list_and_keeps_posting_
  url_in_sync`.

**Verified this session:** this sandbox has no network access, so a
real `pip install` + `pytest` run wasn't possible. Instead: every
touched `.py` file passed `python3 -m py_compile`; the frontend's
`<script type="text/babel">` JSX block was extracted and parsed
cleanly through a locally-available `esbuild` binary (syntax-level
check only, same caveat as before — not a runtime/rendering check);
and the new `overrides_store.py` and `mail_app_store.py` logic
(schema/migration, whitelist AppleScript condition-building,
`force_posting` classification, `extract_posting_urls` behavior) was
manually exercised end-to-end with real inputs outside pytest,
including the actual Haystack digest scenario. **Run `pytest` for real
the first time you're back on a machine with network/deps** — the new
test files above haven't been executed by an actual test runner yet,
only reasoned through and manually spot-checked.

**Update — real `pytest` run confirmed 279/280 passing, one fixed:**
`test_preview_persists_every_listing_link_for_a_multi_listing_digest`
failed (`kind == "application"`, expected `"posting"`). Root cause was
in the test, not the implementation: it calls `_discover_one()` with a
subject ("Software Engineer at Haystack") that `is_job_posting_style_
subject()` correctly returns `False` for — confirmed deliberately by
`test_search_unmatched_messages_reports_force_posting_for_a_
whitelisted_sender_hit`, which asserts that exact non-match — and
`search_unmatched_messages` is mocked in this test, so no real
`force_posting` ever gets computed either. Nothing in the candidate
dict told `discover_new_applications()` to classify it as a posting.
Fix: added `force_posting=True` to that `_discover_one()` call, since
in reality this digest only reaches the queue at all because Part
5.5a's sender whitelist marks LinkedIn Job Alerts as always-posting —
that's the condition this test should have been simulating. No
production code changed.

**Still open:**
- No real-browser click-through of "Always treat this sender as
  postings", the multi-listing link list, or the new Job Postings
  filter box (test suite is now green, but nothing here exercises the
  actual frontend).
- Per-listing company-name labels for `posting_urls` (currently just
  "Listing 1", "Listing 2", ...) — the spec allows this as a fallback,
  but nicer labels would need parsing the digest body's per-listing
  text blocks, which this pass didn't attempt.
- Parts 6 (archive), 7 (stats bar), 8 (filtering) from
  `discoveries-board-v2-spec.md` are still designed but not started —
  Part 8's filter-pill UI, once built, should replace the plain
  `<input>` filter box added here.
